module SidekiqReliableQueue
  ...
  #
  #   queue = SidekiqReliableQueue::WorkingQueue.new("mailer")
  #   queue.each do |job|
  #     job.klass # => 'MyWorker'
  #     job.args # => [1, 2, 3]
  #   end
  ...
  class WorkingQueue
    ...
    def self.all
      Sidekiq.redis { |c| c.keys("queue:*:#{::Sidekiq::ReliableFetcher::WORKING_PREFIX}:*") }
        .sort
        .map { |q| SidekiqReliableQueue::WorkingQueue.new(q) }
    end
